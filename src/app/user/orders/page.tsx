  ...
  return (
    <div className='space-y-2'>
      ...
      <div className='div overflow-x-auto'>
          ...
          <TableBody>
            {orders.data.map((order) => (
              <TableRow key={order.id}>
                ...
                <TableCell>{order.isDelivered && order.paidAt ? formatDateTime(order.paidAt).dateTime : 'Not Delivered'}</TableCell>
                <TableCell>
        ...
        {orders.totalPage > 1 && <Pagination page={Number(page) || 1} totalPages={orders?.totalPage} />}
      </div>
    </div>
  );
}
